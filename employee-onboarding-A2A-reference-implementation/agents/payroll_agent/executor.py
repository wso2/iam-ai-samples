"""
Finance & Payroll Agent Executor — A2A standard pattern with EventQueue.
Identical pattern to ApprovalExecutor.
"""
...
from .agent import PayrollAgent

logger = logging.getLogger(__name__)


class PayrollExecutor(AgentExecutor):
    """Standard A2A executor for Finance & Payroll Agent using EventQueue."""

    def __init__(self, config: dict = None):
        self.agent = PayrollAgent(config)
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        logger.info(f"[PAYROLL_AGENT] Processing: {query[:80]}...")
